...
$(".btnupdpatient").on("click", function () {
    let id = this.id;
    $.ajax({
    method: "POST",
    url: "get-patient.php",
    ...
    success: function (data) {
        ...
        $("#city").val(data.city);
        $("#city").val(data.city);
        $("#BG").val(data.bloodgroup);
        $(`#${data.gender === 'M' ? 'male' : data.gender === 'F' ? 'female' : 'Other'}`).prop("checked", true);
        $("#DOB").val(data.dob);
    },
  });
});
    ...
    success: function (data) {
      ...
      $.each(data, function (index, appointment) {
        ...
        var row =
          ...
          "<td> Pending</td>" +
          "<td>" +
          '<button class=" btn-primary btn-view p-1 border border-dark rounded" data-id="' +
          appointment.id +
          '"><a href="in-patient.php?id=' +
          appointment.id +
          '">Call In</a></button>' +
...
form.addEventListener("input", () => {
  ...
  const isFilled = Array.from(inputs).some(
